...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const reviews = await sql`SELECT r.id AS id, u.name, u.email, u.id as user_id, r.star, r.message,r.product_id FROM reviews r JOIN users u ON u.id = r.user_id where r.product_id = ${Number(id)}`;
